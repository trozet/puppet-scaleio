...
mdm_components = {
  'scaleio_mdm_ips'           => ['/Master MDM/,/Tie-Breakers/p', 'IPs:'],
  'scaleio_tb_ips'            => ['/Tie-Breakers/,//p', 'IPs:'],
  'scaleio_mdm_names'         => ['/Master MDM/,/Tie-Breakers/p', 'Name:'],
  'scaleio_tb_names'          => ['/Tie-Breakers/,//p', 'Name:'],
}
  ...
  Facter.add(name) do
    setcode do
      ...
      mdm_opts.each do |opts|
        cmd = "scli %s --query_cluster --approve_certificate | sed -n '%s' | awk '/%s/ {print($2)}' | tr -d ','" % [opts, selector[0], selector[1]]
